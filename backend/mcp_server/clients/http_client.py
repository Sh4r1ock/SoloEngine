# -*- coding: utf-8 -*-
"""
HTTP Client - 通过Streamable HTTP连接MCP服务器。
...
class HTTPClient(BaseClient):
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.info(f"HTTP client connected: {self.server_info.name}")
            
        except Exception as e:
            await self._cleanup()
            logger.error(f"Failed to connect HTTP client: {e}")
            raise
    ...
    async def _load_capabilities(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"HTTP client initialized: {len(self._tools)} tools, "
    ...
    async def disconnect(self) -> None:
        ...
        self._resources = []
        self._prompts = []
        logger.info(f"HTTP client disconnected: {self.server_info.name}")
